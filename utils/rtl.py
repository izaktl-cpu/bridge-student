...
# ─────────────────────────────────────────────────────────────────────────────
# טוקנייזר לרינדור מפוצל (Label לכל אסימון) — מבטל את "הסלט" בלי תווי-בקרה.
# אשכול לטיני = מספר / טווח / הכרזה / מילה לטינית / סמל חליפה — נשמר כיחידה
# אטומית אחת, כך שטווח כמו 8-9 לא מתהפך ל-9-8.
# ─────────────────────────────────────────────────────────────────────────────
_LTR_CHARS = r'0-9A-Za-z♣♦♥♠+\-=/'
_TOKEN_RE = re.compile(rf'[{_LTR_CHARS}]+|[^\s{_LTR_CHARS}]+')


def is_ltr_token(tok):
    """True אם האסימון הוא אשכול לטיני (מספר/הכרזה/מילה לטינית/סמל)."""
    return bool(re.fullmatch(rf'[{_LTR_CHARS}]+', tok))


def tokens(line):
    """מפצל שורה בודדת לרשימת אסימונים בסדר המקורי (שמאל→ימין),
    כשכל אשכול לטיני נשמר שלם. רווחים לבנים מושמטים (הרינדור מוסיף padx).
